...
const NOTIFICATIONS = [
  {
    ...
    body: "Laura Elliott joined the waitlist for - Laser Refining (Face) .",
    time: "1 week ago",
    clinic: "Chebo Clinic",
    read: false,
  },
  ...
  },
  {
    id: 3,
    title: "New waitlist request",
    body: "Lora Zamias joined the waitlist for - Dermaflux + DermaMPT + Silhouette Plus + Six Star Peel Mini.",
    time: "3 weeks ago",
    clinic: "Chebo Clinic",
    read: false,
  },
  ...
  },
  {
    id: 5,
    title: "New waitlist request",
    body: "Michael Chen joined the waitlist for - HydraFacial Deluxe.",
    time: "1 month ago",
    clinic: "Chebo Clinic",
    read: true,
  },
  {
    ...
    body: "Sarah Johnson confirmed her appointment for Botox Consultation.",
    time: "1 month ago",
    clinic: "Chebo Clinic",
    read: true,
  },
...
export default function DashboardHeader({ title, subtitle, showIcons = true }) {
  ...
  return (
<div className="bg-white w-full flex items-center justify-between md:px-6 px-3 py-4 border-b border-gray-100 z-40 sticky top-0">
            ...
            {openBell && (
              <div className="absolute right-0 top-12 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden animate-fade-in">
                <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                  ...
                  <button
                    onClick={markAllRead}
                    className="text-xs text-custom-blue font-semibold hover:underline"
                  >
                    Mark all as read
                  </button>
                </div>
                <div className="max-h-96 overflow-y-auto divide-y divide-gray-50">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      <div
                        key={n.id}
                        className={`px-4 py-4 transition hover:bg-gray-50 ${!n.read ? "bg-blue-50/40" : ""}`}
                      >
                        {!n.read && (
                          <div className="w-1.5 h-1.5 rounded-full bg-custom-blue float-right mt-1 ml-2 shrink-0" />
                        )}
